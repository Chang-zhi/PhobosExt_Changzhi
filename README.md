...
当 AI 作战方设置 `ConsiderAllOwnersForBaseNode=yes` 时，AI 在判定基地节点是否已建立成功时，会**将所有所属方的建筑**（不限于 AI 自己的）纳入考虑。

**用途：**
- 让 AI 给玩家造建筑。(就是为了这个目的做的这个功能)
- 防止 AI 在被玩家占领建筑后，反复尝试在该位置重建。
- 避免 AI 浪费资源和生产队列去重建已被占领的建筑。

**行为细节：**
- **节点顺序**：AI 严格按照地图设置的基地节点序号由小到大依次建造。序号越小的节点优先级越高。
- **跨所属方判定**：如果节点位置存在任意所属方的同类型建筑，AI 会认为该节点已建立成功，跳过建造。
- **自动重建**：如果节点位置被摧毁（无任何建筑），AI 会自动恢复节点并重新建造。
- **单节点模式**：每帧只保留一个待建造的基地节点，建造完成或建筑放置成功后自动切换到下一个未完成的节点。
- **动态调整**：当已建造完成的节点被摧毁时，AI 会立即中断当前正在建造的其他建筑，优先重建序号更小的节点。
- **触发动作集成**：支持通过触发动作 `560`（添加基地节点）、`561`（移除路径点上的节点）、`562`（移除指定类型的节点）动态管理节点注册表，所有变更会同步到 AI 的建造逻辑中。
...
**捕获时机：**
- 当 `ConsiderAllOwnersForBaseNode=yes` 的 AI 作战方第一次进入 `AI_BaseConstructionUpdate` 逻辑时，会自动将当前 `BaseNodes` 中所有有效节点捕获到内部注册表中。
- 此捕获仅执行一次，后续通过触发动作添加/移除的节点会同步更新注册表。

**存储与持久化：**
- 注册表数据存储在 `HouseExt::ExtData` 中，属于每个 AI 作战方的独立扩展数据。
- 每次游戏存档时，注册表内容（节点类型、坐标、当前建造目标）随存档保存。
- 读档后完整恢复，不会丢失节点状态或建造进度。

**与触发动作的联动：**
- `560`（添加基地节点）：添加节点到游戏 `BaseNodes` 的同时，调用 `AuthorizeBaseNode()` 加入注册表。
- `561`（移除路径点上的节点）：从游戏 `BaseNodes` 删除的同时，调用 `RemoveAuthorizedNodeByCoord()` 从注册表删除。
- `562`（移除指定类型的节点）：从游戏 `BaseNodes` 删除的同时，调用 `RemoveAuthorizedNodeByType()` 从注册表删除。
- 这意味着**所有通过触发动作添加的节点都会被注册表追踪**，不会被 AI 的自动生成逻辑覆盖。

**注册表的作用：**
- 过滤掉遭遇战模式下 AI 自动生成的多余节点（如 NABNKR），确保 AI 只建造任务作者指定的建筑。
- 记录节点的建造顺序和完成状态，支持存档读档后继续未完成的建造序列。
- 当节点被摧毁或占领状态变化时，注册表提供依据来决定是否恢复重建。
