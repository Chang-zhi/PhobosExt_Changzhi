...
**与触发动作的联动：**
- `560`（添加基地节点）：添加节点到游戏中的同时，会同时将该节点加入注册表。
- `561`（移除路径点上的节点）：从游戏中删除的同时，调用会同时将需要移除的节点从注册表删除。
- `562`（移除指定类型的节点）：从游戏中删除的同时，调用会同时将需要移除的节点从注册表删除。
- 这意味着**所有通过触发动作添加的节点都会被注册表追踪**，不会被 AI 的自动生成逻辑覆盖。

**注册表的作用：**
- 过滤掉遭遇战模式下 AI 自动生成的多余节点，确保 AI 只建造任务作者指定的建筑。
- 记录节点的建造顺序和完成状态，支持存档读档后继续未完成的建造序列。
- 当节点被摧毁或占领状态变化时，注册表提供依据来决定是否恢复重建。

> **已知不兼容与限制：**
> 1. **与触发动作 30（自动建设基地）不兼容**：本功能接管了 AI 的基地节点建造逻辑，会强制严格按注册表顺序依次建造。即使同时开启自动建设基地模式，建筑也不会自动更换位置建造。
> 2. **开启时不要让 AI 造围墙**：围墙会会扰乱本功能的单节点控制，导致节点队列卡死。请确保 AI 的基地节点中不包含围墙类型。
