...
> **兼容性说明：**
> 1. **与触发动作 30(自动建设基地)不兼容**：本功能接管了 AI 的基地节点建造逻辑，强制按注册表顺序和数据依次建造。即使开启自动建设基地模式 AI 也不会更换建筑位置。
> 2. **开启时不要让 AI 造围墙**：围墙会扰乱控制，可能导致节点队列卡死。请确保 AI 的基地节点中不包含围墙类型。
> 3. **动作 560 的立即建造参数**：当 `BaseNodeCrossOwners=yes` 时，使用`立即建造`会将节点插入注册表头部，确保 AI 在下帧优先建造该节点。
