...
- **触发动作集成**：支持通过**本dll的触发动作** `560`（添加基地节点）、`561`（移除路径点上的节点）、`562`（移除指定类型的节点）动态管理节点注册表，所有变更会同步到 AI 的建造逻辑中。
...
**捕获时机：**
- 当 `BaseNodeCrossOwners=yes` 的 AI 作战方第一次进入逻辑时，会自动将当前基地节点捕获到内部注册表中。
- 此捕获仅执行一次，后续通过触发动作添加/移除的节点会同步更新注册表。

**与触发动作的联动：**
- `560`（添加基地节点）：添加节点到游戏中的同时，将该基地节点加入注册表。
- `561`（移除路径点上的节点）：从游戏中删除的同时，将对应基地节点从注册表删除。
- `562`（移除指定类型的节点）：从游戏中删除的同时，将对应基地节点从注册表删除。
- 这意味着**所有通过触发动作添加的节点都会被注册表追踪**，可以放心使用。
