...
- `577` – 建立作战小队(考虑限制)... （参数：小队索引）

**注意事项：**

创建一个指定的作战小队(归属于小队所属方)。
若当前已存在的作战小队实例数量已达到上限（ 作战小队 里的 最大 ），则无法继续创建。
...
577=建立作战小队(考虑限制)... (PhobosExt),0,0,7,0,0,0,0,0,0,建立一个指定的作战小队(归属于小队所属方)。通常会直接从已有单位中招募。如果没有足够的单位来组建小队，AI会通过生产补足(自动建造)。当所有成员就位时，小队即自动建立。若当前已存在的作战小队实例数量已达到上限("作战小队"里的"最大")，则无法继续创建。,0,1,577,1
...
```
