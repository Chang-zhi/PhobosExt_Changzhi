...
| 572 | 将路径点范围内除指定类型外的所有科技类型关联到指定标签... | 路径点索引，科技类型ID，标签索引，范围（单元格），布尔值 |
| 573 | 将路径点范围内触发所属方的除指定类型外的所有科技类型关联到指定标签... | 路径点索引，科技类型ID，标签索引，范围，布尔值 |
...
- `560` – 在指定路径点添加基地节点...（参数：所属方国家索引，路径点编号，建筑类型索引，布尔值）
- `561` – 移除指定路径点上的所有基地节点...（参数：所属方国家索引，路径点编号）
- `562` – 移除特定所属方所有指定建筑类型的基地节点...（参数：所属方国家索引，建筑类型索引）

**注意事项：**
对于动作 `560`,布尔值用来指定是否立即开始建造指定建筑, 为真时AI会立即开始建造指定建筑并清空建筑生产序列。
...
### 11. 建立作战小队(考虑限制)
**触发动作说明：**
- `577` – 建立作战小队(考虑限制)... （参数：小队索引，布尔值，布尔值，布尔值）

**注意事项：**
- **上限限制(参数2)**：启用后，若当前实例数已达到"作战小队"里的"最大"值，则不创建。
- **区域检查(参数3)**：启用后，检测所属方与敌对方之间是否存在区域连通路径，无连接则跳过。
- **连通模式(参数4)**：仅在区域检查启用时生效。
  - `0`（假）：小队内任一兵种可以抵达目的地即可。
  - `1`（真）：小队内全部兵种都需要可以抵达目的地才行。
...
552=路径点上存在所属方的指定科技类型... (PhobosExt),49,46,0,0,当路径点上存在触发所属方的指定科技类型时，此事件被满足。(若科技类型为建筑，则每一格都会检测，支持ares的自定义Foundation),0,1,552,1
553=路径点上不存在所属方的指定科技类型... (PhobosExt),49,46,0,0,当路径点上不再存在触发所属方的指定科技类型时，此事件被满足。(若科技类型为建筑，则每一格都会检测，支持ares的自定义Foundation),0,1,553,1
...
```
...
560=在指定路径点添加基地节点... (PhobosExt),0,0,2,30,8,15,0,0,0,在指定路径点添加指定所属方的基地节点。(布尔值用来指定是否立即开始建造指定建筑),0,1,560,1
...
564=将路径点上的指定科技类型关联到指定标签... (PhobosExt),-4,46,38,30,15,0,0,0,0,将指定路径点上的指定科技类型关联到指定指定标签。布尔值用来指定是否优先创建一个新的标签实例(TagClass)。(若科技类型为建筑，则每一格都会检测，支持ares的自定义Foundation，详细细节请见"说明文档.html"里的相关介绍),0,1,564,1
565=将路径点上指定所属方的指定科技类型关联到指定标签... (PhobosExt),-4,46,38,30,2,15,0,0,0,将指定路径点上指定所属方的指定科技类型关联到指定指定标签。布尔值用来指定是否优先创建一个新的标签实例(TagClass)。(若科技类型为建筑，则每一格都会检测，支持ares的自定义Foundation，详细细节请见"说明文档.html"里的相关介绍),0,1,565,1
...
572=将路径点范围内除指定类型外的所有科技类型关联到指定标签... (PhobosExt),-4,46,38,30,6,15,0,0,0,将指定路径点指定范围内除指定类型外的所有科技类型关联到指定标签。(圆形范围)\n布尔值用来指定是否优先创建一个新的标签实例。(详细细节请见"说明文档.html"里的相关介绍),0,1,572,1
573=将路径点范围内触发所属方的除指定类型外的所有科技类型关联到指定标签... (PhobosExt),-4,46,38,30,6,15,0,0,0,将指定路径点指定范围内触发所属方的除指定类型外的所有科技类型关联到指定标签。(圆形范围)\n布尔值用来指定是否优先创建一个新的标签实例。(详细细节请见"说明文档.html"里的相关介绍),0,1,573,1
574=更新所有建筑的动画... (PhobosExt),0,0,0,0,0,0,0,0,0,更新所有建筑的动画，用于处理建筑改变所属后动画会停止播放的问题。(但遥控坦克的状态仍然不会更新),0,1,574,1
575=更新关联建筑的动画... (PhobosExt),0,0,0,0,0,0,0,0,0,更新关联建筑的动画，用于处理建筑改变所属后动画会停止播放的问题。(但遥控坦克的状态仍然不会更新),0,1,575,1
576=更新指定所属方所有建筑的动画... (PhobosExt),0,0,2,0,0,0,0,0,0,更新指定所属方所有建筑的动画，用于处理建筑改变所属后动画会停止播放的问题。(但遥控坦克的状态仍然不会更新),0,1,576,1
577=建立作战小队(考虑限制)... (PhobosExt),0,0,7,15,15,15,0,0,0,根据指定索引创建小队，可选启用限制:\n上限限制(参数2):依据"作战小队"中的"Max"值，超出则不创建。\n区域检查(参数3):启用后检测与敌对方的区域连通性。\n连通模式(参数4):0=任一兵种可以抵达目的地即可，1=全部兵种都需要可以抵达目的地(仅在参数3启用时生效),0,1,577,1
...
```
