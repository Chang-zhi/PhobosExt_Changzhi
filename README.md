...
5.`TemporalAOE.Enable=yes` 在 OpenTopped 开放式载具（如要塞）内作为乘员使用时存在异常，暂不建议在此场景下使用。
6.`TemporalAOE.Enable=yes` 将建筑作为副目标冻结时，建筑的附属动画（烟囱、生产等）可能出现未暂停/未半透明的表现。**这仅是外观问题，不影响建筑的冻结和抹除功能。**
...
TemporalAOE.SecondaryWeight=               ; double，默认 1.0 — 副目标冻结时间贡献权重
...
- **时间计算公式**：`额外冻结帧数 = (10 × 副目标HP × SecondaryWeight) ÷ 武器Damage`。每个副目标的贡献独立累加。
  - 例如：副目标 HP=200，`SecondaryWeight=1.0`，武器伤害=100，则每个副目标增加 `10×200×1.0÷100 = 20` 帧。
- **进出范围**：单位进入或离开范围时，冻结所需总时间会自动重新计算，已消耗的进度不会丢失。   
~~（你是怎么离开的?）~~
- **要塞兼容**：~~超时空兵在 OpenTopped 要塞内作为乘员开火时，AOE 正常生效，且要塞本身不会受到冻结影响。~~
**（已知 Bug：要塞内使用存在崩溃和状态机误判风险。现在进入开放式载具后 AOE 功能自动禁用。）**
- **攻击者免冻**：正在使用超时空武器的单位不会被选为副目标，防止多个 AOE 攻击者之间互相冻结。
- **超时空武器拦截**：所有 `Temporal=yes` 的超时空武器（无论是否设置 Exclusive/AOE）均无法将已被 AOE 锁定的**主目标和副目标**选为攻击对象(防止bug)。
- **自动清理**：系统每帧检测所有被冻结的单位，自动解除以下情况的冻结：攻击者已死亡/被冻结/停止攻击、副目标已死亡/离场。额外每 15 帧进行一次全图扫描，清除任何异常残留的孤立冻结状态。  
(被逼无奈才做全局处理, 一直有残留问题)
...
- `578` – 将路径点范围内指定所属方的可招募单位加入已存在的作战小队... (参数：小队索引，路径点，范围，所属方国家索引)
...
**动作 578 (招募单位到已有小队)：**
- 将指定路径点指定范围内、指定所属方的所有可招募地面单位(FootClass)加入一个已存在的作战小队实例。
- 如果没有该类型的已有实例，则无动作。
- 只会招募满足以下条件的单位：属于指定所属方、未加入其他小队、可被招募(`CanBeRecruited`)、在路径点指定范围内。
...
### 兼容性说明：
> 1. **与触发动作 30(自动建设基地)不兼容**：   
本功能接管了 AI 的基地节点建造逻辑，强制按注册表顺序和数据依次建造。即使开启自动建设基地模式 AI 也不会更换建筑位置。
> 2. **开启时不要让 AI 造围墙**：    
围墙会扰乱控制，可能导致节点队列卡死。请确保 AI 的基地节点中不包含围墙类型。
> 3. **动作 560 的立即建造参数**：    
当 `BaseNodeCrossOwners=yes` 时，使用`立即建造`会将节点插入注册表头部，确保 AI 在下帧优先建造该节点。  
...
```ini
[General]
BerzerkRestoreClearTarget=   ; boolean(布尔值)，默认 false
```

启用时，当单位从混乱状态恢复会自动清除当前攻击目标，停止移动，切换至 `Guard` 状态，用于防止单位乱跑。

#### 14.2 弹头混乱缩减

```ini
[WarheadType]
BerserkReduce=               ; integer(整数)，默认 0
```

当单位受到带有此标签的弹头攻击时（无论是否造成伤害），减少目标的混乱持续时间。

- **正值**：减少对应帧数的混乱时间（加速清醒）。
- **负值**：延长对应帧数的混乱时间。
- 如果减少后的剩余时间 ≤ 0，则**立即清除混乱状态**，单位强制停止移动、进入 `Guard` 模式并清除攻击目标，防止乱跑。
