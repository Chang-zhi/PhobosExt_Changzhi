...
{
	// Reset the custom briefing for a new scenario. Called via ScaffoldTypeRegistry
...
void ScenarioExt::ExtData::LoadFromStream(ScaffoldStreamReader& Stm)
{
	Extension<ScenarioClass>::LoadFromStream(Stm);
	this->Serialize(Stm);
}

void ScenarioExt::ExtData::SaveToStream(ScaffoldStreamWriter& Stm)
{
...
DEFINE_HOOK(0x68945B, ScenarioClass_Save_Suffix, 0x8)
{
	...
	ScaffoldByteStream saver(sizeof(*Ext));
